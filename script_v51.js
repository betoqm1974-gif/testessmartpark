const POS_ANSWERS={
  t1_q1:'F',
  t2_q1:'B', t2_q2:'C', t2_q3:'A', t2_q4:'B',
  t3_q1:'D',
  t4_q1:'B',
  t5_q1:'C',
  t6_q1:['7','sete'],
...
const PRE_ANSWERS={
  p1_q1:'B',
  p2_q1:'A', p2_q2:'B', p2_q3:'D', p2_q4:'C',
  p3_q1:'A',
  p4_q1:'D',
  p5_q1:'C',
  p6_q1:['8','oito'],
...
function getCurrentAnswers(){
  return (document.body && document.body.dataset && document.body.dataset.test==='pre') ? PRE_ANSWERS : POS_ANSWERS;
}
function getCurrentTestName(){
  return (document.body && document.body.dataset && document.body.dataset.test==='pre') ? 'Pré-Teste SmartPark' : 'Pós-Teste SmartPark';
}
function getEmailSubjectTestName(){
  return (document.body && document.body.dataset && document.body.dataset.test==='pre') ? 'Pré-Teste' : 'Pós-Teste';
}
...
function etiquetaPergunta(name){
  const mapa={
    ...
    t5_q1:'Exercício 5 - O Percurso do Robô',
    t6_q1:'Exercício 6 - Entregas no Festival',
    t7_q1:'Exercício 7 - RoboLiga',
    p1_q1:'Exercício 1 - Descargas no Porto',
    p2_q1:'Exercício 2.1 - Vizinho da Beatriz',
    p2_q2:'Exercício 2.2 - Gabinete com 2 corredores',
    p2_q3:'Exercício 2.3 - Gabinete do Carlos',
    p2_q4:'Exercício 2.4 - Gabinetes com 1 vizinho',
    p3_q1:'Exercício 3 - Mensagem do Arquivo Digital',
    p4_q1:'Exercício 4 - Cartões e Envelopes',
    p5_q1:'Exercício 5 - O Percurso do Robô',
    p6_q1:'Exercício 6 - Entregas no Festival',
    p7_q1:'Exercício 7 - RoboLiga'
  };
...
function obterResumoResultados(){
  ...
  const linhas=[];
  for(const [name,ans] of Object.entries(getCurrentAnswers())){
    total++;
...
function gerarCsvResultados(aluno,resumo){
  const linhas=[];
  linhas.push(['Teste',getCurrentTestName()]);
...
function gerarTabelaHtmlResultados(aluno,resumo){
  ...
  return `
    <h2>Resultados - ${htmlEscape(getCurrentTestName())}</h2>
...
async function enviarResultados(){
  ...
  const nomeAlunoAssunto = (aluno.nome || '').trim();
  const assunto = nomeAlunoAssunto ? `Resultados ${getEmailSubjectTestName()} - ${nomeAlunoAssunto}` : `Resultados ${getEmailSubjectTestName()}`;
  ...
  addHidden('Teste', getCurrentTestName());
...
function mostrarPopupEnviado(){
  let modal=document.getElementById('popup-enviado');
  if(!modal){
    modal=document.createElement('div');
    modal.id='popup-enviado';
    modal.className='send-modal';
    modal.innerHTML=`
      <div class="send-modal-card" role="dialog" aria-modal="true" aria-labelledby="popup-enviado-titulo">
        <h2 id="popup-enviado-titulo">Teste enviado</h2>
        <button type="button" onclick="sairParaInicioTeste()">Sair</button>
      </div>`;
...
function sairParaInicioTeste(){
  window.location.href=(document.body && document.body.dataset && document.body.dataset.test==='pre') ? 'pre_teste.html' : 'pos_teste.html';
}
